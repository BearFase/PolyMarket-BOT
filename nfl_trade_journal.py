#!/usr/bin/env python3
"""Permanent, append-only journal of every observed NFL trade on Polymarket US.

Instrumentation only. It records what the trade stream reports and nothing
more: no analysis, no signals, no edge logic. It mirrors the immutable MLB
journal in mlb_research.py — an append-only table whose UPDATE and DELETE are
blocked by triggers, idempotent inserts, WAL — with three deliberate
differences.

Every trade, not a subset.
    The MLB journal receives taker BUY executions on moneyline markets at or
    above the display threshold; big_money_tape discards everything else before
    that journal sees it. This journal is fed ahead of those filters, so it
    records every NFL trade the stream delivers — sells, spreads and totals
    included. Reconstructing a price path later needs the whole tape.
...
Deduplication on the exchange's trade id.
    ...
    so two equal-size fills at one price inside a burst derive the same key and
    the second is silently discarded. The exchange id was present, and unique,
    on all 63,571 of those rows. The derived key is still stored, for audit and
    for joining against the tape and the MLB journal.

No canonical-game resolution at write time.
    Rows carry the Polymarket US event and market slugs. Linking them to a
    canonical game_uuid happens at analysis time, through the registry's
    source_links. That keeps the write path independent of the NFL registry, so
    registry locks or schema changes can never stall or break capture.

Coverage begins at `coverage_started_at` in journal_meta, written once, when a
capture session first starts. Nothing earlier is backfilled: the dashboard tape
prunes itself to five rows per event and is not a record of what traded. Every
capture start appends a row to capture_sessions, so outages show up as gaps.

Fail-safe: big_money_tape calls `journal_trade`, which never raises. A broken
journal costs journal rows — never display rows, the MLB journal, or the
stream.

    python nfl_trade_journal.py status
"""
...
DEFAULT_DB = HERE / "nfl_trade_journal.db"
SCHEMA_VERSION = 1
...
CREATE TABLE IF NOT EXISTS nfl_trade_observations(
...
CREATE INDEX IF NOT EXISTS nfl_trades_market_time
  ON nfl_trade_observations(market_slug, trade_timestamp_utc);
CREATE INDEX IF NOT EXISTS nfl_trades_event_time
  ON nfl_trade_observations(event_slug, trade_timestamp_utc);
CREATE INDEX IF NOT EXISTS nfl_trades_derived_key
  ON nfl_trade_observations(derived_trade_key);
CREATE TRIGGER IF NOT EXISTS nfl_trade_observations_no_update
  BEFORE UPDATE ON nfl_trade_observations
  BEGIN SELECT RAISE(ABORT, 'NFL trade observations are immutable'); END;
CREATE TRIGGER IF NOT EXISTS nfl_trade_observations_no_delete
  BEFORE DELETE ON nfl_trade_observations
  BEGIN SELECT RAISE(ABORT, 'NFL trade observations are immutable'); END;
...
CREATE TRIGGER IF NOT EXISTS journal_meta_coverage_no_update
  ...
  WHEN OLD.key IN ('coverage_started_at', 'coverage_note')
...
def stable_uuid(value: str) -> str:
    return str(uuid.uuid5(TRADE_NAMESPACE, f"nfl-trade:{value}"))
...
def build_observation(trade: dict, meta: dict, session_uuid: str | None,
    ...
    return {
        "observation_uuid": stable_uuid(dedupe_key),
...
class NFLTradeJournal:
    """Append-only store. Every write is idempotent on dedupe_key."""
    ...
    def record(self, trade: dict, meta: dict, session_uuid: str | None = None) -> bool:
        ...
        row = build_observation(trade, meta, session_uuid, utc_now())
        columns = ",".join(row)
        marks = ",".join("?" for _ in row)
        with self.connect() as db:
            before = db.total_changes
            db.execute(f"INSERT OR IGNORE INTO nfl_trade_observations({columns}) "
...
_journals: dict[str, NFLTradeJournal] = {}
_sessions: dict[str, str] = {}
_stats = {"inserted": 0, "duplicate": 0, "failed": 0}
_last_error_report = float("-inf")
_last_stats_log: float | None = None


def _journal(path: Path | str) -> NFLTradeJournal:
    key = str(Path(path))
    with _lock:
        existing = _journals.get(key)
    if existing is not None:
        return existing
    created = NFLTradeJournal(path)
    with _lock:
...
def start_capture_session(path: Path | str, note: str | None = None,
                          on_error: Callable[[str], Any] | None = None) -> str | None:
    """Record that capture is live. Never raises; None means it failed."""
    global _last_stats_log
    try:
        session = _journal(path).start_session(note)
    except Exception as exc:
        _safe_call(on_error, f"NFL trade journal could not start a capture session: {exc}")
        return None
    with _lock:
...
def journal_trade(path: Path | str, trade: dict, meta: dict,
                  on_error: Callable[[str], Any] | None = None,
                  on_stats: Callable[[str], Any] | None = None) -> bool | None:
    """Append one observed trade. Never raises.
    ...
    """
    global _last_error_report, _last_stats_log
    try:
        with _lock:
            session = _sessions.get(str(Path(path)))
        if session is None:
            session = start_capture_session(
                path, note="started by first observed trade", on_error=on_error)
        inserted = _journal(path).record(trade, meta, session)
        with _lock:
            _stats["inserted" if inserted else "duplicate"] += 1
    except Exception as exc:
        with _lock:
            _stats["failed"] += 1
        now = time.monotonic()
        if now - _last_error_report >= ERROR_REPORT_INTERVAL_SECONDS:
            _last_error_report = now
            _safe_call(on_error, f"NFL trade journal write failed: {exc}")
        return None
    try:
        now = time.monotonic()
        if _last_stats_log is not None and now - _last_stats_log >= STATS_LOG_INTERVAL_SECONDS:
            _last_stats_log = now
            _safe_call(on_stats, "NFL trade journal: " + stats_line())
    except Exception:
        pass
    return inserted
...
def read_status(path: Path | str = DEFAULT_DB) -> dict:
    """Coverage and integrity counts. Never creates or writes the database."""
    path = Path(path)
    if not path.exists():
        return {"database": str(path), "started": False}
    with _open_read_only(path) as db:
        ...
        flags: dict[str, int] = {}
        for (value,) in db.execute("SELECT source_quality_flags_json FROM "
                                   "nfl_trade_observations WHERE source_quality_flags_json <> '[]'"):
            for flag in json.loads(value):
                flags[flag] = flags.get(flag, 0) + 1
        paths: set[str] = set()
        for (value,) in db.execute("SELECT raw_key_paths_json FROM nfl_trade_observations "
        ...
        return {
            ...
            "observations": one("SELECT COUNT(*) FROM nfl_trade_observations"),
            "with_exchange_trade_id": one(
                "SELECT COUNT(websocket_trade_id) FROM nfl_trade_observations"),
            "distinct_exchange_trade_ids": one(
                "SELECT COUNT(DISTINCT websocket_trade_id) FROM nfl_trade_observations"),
            "first_trade_utc": one("SELECT MIN(trade_timestamp_utc) FROM nfl_trade_observations"),
            "last_trade_utc": one("SELECT MAX(trade_timestamp_utc) FROM nfl_trade_observations"),
            "distinct_markets": one("SELECT COUNT(DISTINCT market_slug) FROM nfl_trade_observations"),
            "distinct_events": one("SELECT COUNT(DISTINCT event_slug) FROM nfl_trade_observations"),
            "by_market_type": grouped(
                "SELECT market_type, COUNT(*) FROM nfl_trade_observations GROUP BY 1"),
            "by_taker_intent": grouped(
                "SELECT taker_intent, COUNT(*) FROM nfl_trade_observations GROUP BY 1"),
            "by_pregame_classification": grouped(
                "SELECT pregame_classification, COUNT(*) FROM nfl_trade_observations GROUP BY 1"),
            # Fills that MLB's derived-key dedupe would have merged into one row.
            "derived_keys_shared_by_distinct_trades": one(
                "SELECT COUNT(*) FROM (SELECT derived_trade_key FROM nfl_trade_observations "
                "GROUP BY 1 HAVING COUNT(*) > 1)"),
            "rows_sharing_a_derived_key": one(
                "SELECT COALESCE(SUM(c), 0) FROM (SELECT COUNT(*) c FROM nfl_trade_observations "
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="NFL permanent trade journal.")
    parser.add_argument("command", choices=("status",))
    parser.add_argument("--db", default=str(DEFAULT_DB))
    args = parser.parse_args(argv)
    print(json.dumps(read_status(args.db), indent=2))
    return 0
